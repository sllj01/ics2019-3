...
make_group(gp1,
    ...
    EXW(and, 1), EXW(sub, 1), EXW(xor, 1), EXW(cmp, 1))//ADD,OR, ADC, SBB, AND, SUB, XOR, CMP

/* 0xc0, 0xc1, 0xd0, 0xd1, 0xd2, 0xd3 */
make_group(gp2,
    EXW(rol, 1), EMPTY, EMPTY, EMPTY,
    EXW(shl, 1), EX(shr), EMPTY, EXW(sar, 1))//ROL, ROR, RCL, RCR, SHL, SHR, NONE, SAR
...
make_group(gp5,
    ...
    EX(jmp_rm), EMPTY, EX(push), EMPTY)//INC, DEC, CALL, CALL, JMP, JMP, PUSH
